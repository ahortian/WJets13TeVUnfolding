...
void setAndDrawTPad(TString canvasName, TPad *plot, int plotNumber, int numbOfGenerator)
{
    if (numbOfGenerator == 1) {
        ...
        if (plotNumber == 1) {
            plot->SetPad(0.01, 0.565, 0.99, 0.99);
            plot->SetTopMargin(0.136);
            plot->SetBottomMargin(0.007);
        }
        else if (plotNumber == 2) {
            plot->SetPad(0.01, 0.405, 0.99, 0.565);
            plot->SetTopMargin(0.0);
            plot->SetBottomMargin(0.0);
        }
        else if (plotNumber == 3) {
            plot->SetPad(0.01, 0.245, 0.99, 0.405);
            plot->SetTopMargin(0.0);
            plot->SetBottomMargin(0.0);
        }
        else if (plotNumber == 4) {
            plot->SetPad(0.01, 0.01, 0.99, 0.245);
            plot->SetTopMargin(0.0);
            plot->SetBottomMargin(0.33);
        }
    }
...
void customizeLegend(TLegend *legend, int genNumb, int numbOfGenerator)
{
	...
	if (numbOfGenerator == 3) legend->SetTextSize(.128);
...
{
    hSyst->GetYaxis()->SetRangeUser(0.25, 1.75);
    //hSyst->GetYaxis()->SetRangeUser(0.41, 1.59);
    //hSyst->GetYaxis()->SetRangeUser(0.11, 1.89);  //for DeltaR and Fourthjet_pT uncomment

    hSyst->GetYaxis()->SetNdivisions(507);
	hSyst->GetYaxis()->SetTitle(yTitle);
	if (numbOfGenerator ==3){
		if (genNum == 1) hSyst->GetYaxis()->SetTitleSize(0.117);
		if (genNum == 2) hSyst->GetYaxis()->SetTitleSize(0.117);
		hSyst->GetYaxis()->SetLabelSize(0.163);
...
TCanvas* makeCrossSectionPlot(TString lepSel, TString variable, bool doNormalized, TH1D *hStat, TH2D *hCovSyst, TH1D *hGen1, TH1D *hGen2, TH1D* hGen3, double integratedLumi)
{
  ...
  //gStyle->SetOptStat(0);
    ...
    */
    ...
    hSyst->GetYaxis()->SetLabelSize(0.070);
    }
    else{
    hSyst->GetYaxis()->SetLabelSize(0.060);
    }
    ...
    if (canvasName.Contains("dRptmin100LepCloseJetCo300dR04_Zinc1jet")) {
    hSyst->GetYaxis()->SetLabelSize(0.070);
    }
    hSyst->DrawCopy("e");
    ...
    if (hGen2) latexLabel->SetTextSize(0.05);
    if (hGen3) latexLabel->SetTextSize(0.061);
    
	
    latexLabel->SetLineWidth(2);
    latexLabel->SetTextFont(61);
    if (canvasName.Contains("dPhiLepJet1")){
    latexLabel->DrawLatex(0.18,0.23,"CMS");
    }
    else{
    latexLabel->DrawLatex(0.18,0.25,"CMS");
    }
    ...
    if (canvasName.Contains("FirstJetPt50")){
		...
		if (numbOfGenerator == 3) latexLabel->DrawLatex(0.18,0.21-0.10,"p_{T}^{jet} > 30 GeV, |y^{jet}| < 2.4 ");
    ...
    if (canvasName.Contains("FirstJetPt_Zinc1jet")||canvasName.Contains("JetsHT_Zinc1jet")||canvasName.Contains("FirstJetAbsRapidity_Zinc1jet")||canvasName.Contains("dPhiLepJet1_Zinc1jet")||canvasName.Contains("dRptmin100LepCloseJetCo300dR04_Zinc1jet")){
    if (lepSel == "SMu") latexLabel->DrawLatex(0.18,0.21-0.17,"W(#mu#nu) + #geq 1-jet");
	...
	if (numbOfGenerator == 3) ytitle->DrawLatex(0.008,0.89,strYtitle.c_str());
	else ytitle->DrawLatex(0.008,0.91,strYtitle.c_str());
